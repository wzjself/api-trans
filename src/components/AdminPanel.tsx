  ...
  return (
    <div className="space-y-12 pb-20">
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-7 gap-4">
        ...
        <div className="p-4 rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900/50"><div className="text-xs text-zinc-500">14天请求数</div><div className="text-2xl font-bold tracking-tight">{platformSummary.totalRequests.toLocaleString()}</div></div>
        <div className="p-4 rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900/50"><div className="text-xs text-zinc-500">14天总消耗</div><div className="text-2xl font-bold tracking-tight">{formatTokensInMillions(platformSummary.totalTokens)}</div><div className="text-[10px] text-zinc-400 mt-1">单位：M</div></div>
      ...
      <div className="space-y-6">
        ...
        <div className="p-6 rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900/50 space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <button onClick={saveAnnouncement} disabled={isSavingAnnouncement} className="px-6 py-2 text-sm font-medium text-white bg-zinc-900 dark:bg-zinc-100 dark:text-zinc-900 rounded-xl">{isSavingAnnouncement ? '保存中...' : '保存公告与弹窗设置'}</button>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-6">
        ...
        <div className="overflow-x-auto rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900/50">
          <table className="w-full text-left text-xs min-w-[1080px]">
            <thead><tr className="border-b border-zinc-200 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-900/50"><th className="px-3 py-2 font-medium text-zinc-500">用户</th><th className="px-3 py-2 font-medium text-zinc-500">角色</th><th className="px-3 py-2 font-medium text-zinc-500">永久额度</th><th className="px-3 py-2 font-medium text-zinc-500">卡类型</th><th className="px-3 py-2 font-medium text-zinc-500">每日额度</th><th className="px-3 py-2 font-medium text-zinc-500">今日已用</th><th className="px-3 py-2 font-medium text-zinc-500">到期时间（北京时间）</th><th className="px-3 py-2 font-medium text-zinc-500">累计消耗</th><th className="px-3 py-2 font-medium text-zinc-500">活跃密钥</th><th className="px-3 py-2 font-medium text-zinc-500">注册时间</th><th className="px-3 py-2 font-medium text-zinc-500 text-right">操作</th></tr></thead>
            <tbody className="divide-y divide-zinc-200 dark:divide-zinc-800">
              {users.map((user) => <tr key={user.uid} className="group hover:bg-zinc-50/50 dark:hover:bg-zinc-900/30 transition-colors align-top"><td className="px-3 py-2"><div className="flex items-start gap-2"><div className="w-7 h-7 rounded-full bg-zinc-100 dark:bg-zinc-800 flex items-center justify-center shrink-0 mt-0.5"><UserIcon className="w-3.5 h-3.5 text-zinc-500" /></div><div className="min-w-0"><div className="font-medium truncate max-w-[140px]">{user.email}</div><div className="text-[10px] text-zinc-500 font-mono truncate max-w-[160px]">{user.uid}</div></div></div></td><td className="px-3 py-2"><div className="flex items-center gap-1">{user.role === 'admin' ? <Shield className="w-3.5 h-3.5 text-amber-500" /> : <UserIcon className="w-3.5 h-3.5 text-zinc-400" />}<span className={cn('capitalize', user.role === 'admin' ? 'text-amber-600 dark:text-amber-400' : 'text-zinc-600 dark:text-zinc-400')}>{user.role}</span></div></td><td className="px-3 py-2 font-mono text-[11px]">{Number(user.balance || 0).toLocaleString()}</td><td className="px-3 py-2">{user.quotaType || 'none'}</td><td className="px-3 py-2 font-mono text-[11px]">{Number(user.dailyQuota || 0).toLocaleString()}</td><td className="px-3 py-2 font-mono text-[11px]">{Number(user.usedToday || 0).toLocaleString()}</td><td className="px-3 py-2 text-[11px] text-zinc-500 whitespace-nowrap">{user.quotaExpiresAt ? new Date(new Date(user.quotaExpiresAt).getTime() + 8 * 60 * 60 * 1000).toISOString().slice(0, 16).replace('T', ' ') : '-'}</td><td className="px-3 py-2 font-mono text-[11px]">{Number(user.totalUsedTokens || 0).toLocaleString()}</td><td className="px-3 py-2">{Number(user.apiKeyCount || 0)}</td><td className="px-3 py-2 text-[11px] text-zinc-500 whitespace-nowrap">{user.createdAt ? new Date(new Date(user.createdAt).getTime() + 8 * 60 * 60 * 1000).toISOString().slice(0, 16).replace('T', ' ') : '-'}</td><td className="px-3 py-2 text-right"><div className="flex justify-end gap-2 whitespace-nowrap"><button onClick={() => { setEditingUser(user); setNewBalance(user.balance); }} className="text-[11px] font-medium text-zinc-500 hover:text-zinc-900 dark:hover:text-zinc-100 transition-colors">编辑余额</button><button onClick={() => setPendingDeleteUser(user)} className="text-[11px] font-medium text-red-500 hover:text-red-600 transition-colors">删除账号</button></div></td></tr>)}
            </tbody>
          </table>
        </div>
      </div>

      {editingUser && <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm"><div className="w-full max-w-md p-8 rounded-3xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 shadow-2xl space-y-6"><div className="space-y-2"><h3 className="text-xl font-bold tracking-tight">编辑用户余额</h3><p className="text-sm text-zinc-500">修改 {editingUser.email} 的永久额度。</p></div><div className="space-y-1.5"><label className="text-xs font-medium text-zinc-500 uppercase tracking-wider">当前余额: {editingUser.balance.toLocaleString()}</label><input type="number" className="w-full px-4 py-3 text-sm rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 font-mono" value={newBalance} onChange={(e) => setNewBalance(parseInt(e.target.value || '0'))} /></div><div className="flex gap-3"><button onClick={() => setEditingUser(null)} className="flex-1 py-3 text-sm font-medium rounded-xl border border-zinc-200 dark:border-zinc-800">取消</button><button onClick={updateUserBalance} className="flex-1 py-3 text-sm font-medium text-white bg-zinc-900 dark:bg-zinc-100 dark:text-zinc-900 rounded-xl">保存修改</button></div></div></div>}

      {pendingDeleteUser && <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm"><div className="w-full max-w-md p-8 rounded-3xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 shadow-2xl space-y-6"><div className="space-y-3 text-center"><div className="w-14 h-14 mx-auto rounded-full bg-red-50 dark:bg-red-900/20 flex items-center justify-center"><AlertTriangle className="w-7 h-7 text-red-500" /></div><div><h3 className="text-xl font-bold tracking-tight">确认删除账号</h3><p className="text-sm text-zinc-500 mt-2">将要删除：<span className="font-medium text-zinc-900 dark:text-zinc-100">{pendingDeleteUser.email}</span></p><p className="text-xs text-zinc-400 mt-1 break-all">{pendingDeleteUser.uid}</p></div><p className="text-sm text-red-500">删除后该用户的密钥和使用记录也会一并清理，不可恢复。</p></div><div className="flex gap-3"><button type="button" onClick={() => setPendingDeleteUser(null)} className="flex-1 py-3 text-sm font-medium rounded-xl border border-zinc-200 dark:border-zinc-800 hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-all">取消</button><button type="button" onClick={confirmDeleteUser} disabled={isDeletingUser} className="flex-1 py-3 text-sm font-medium text-white bg-red-600 rounded-xl hover:opacity-90 disabled:opacity-50 transition-all">{isDeletingUser ? '删除中...' : '确认删除'}</button></div></div></div>}
    </div>
  );
};
