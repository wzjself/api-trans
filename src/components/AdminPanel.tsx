...
export const AdminPanel: React.FC = () => {
  ...
  const generateCode = async () => {
    ...
    try {
      ...
      await dataService.addCode({ code, type: newCode.type, value: newCode.value, durationDays: newCode.durationDays });
      loadAdminData();
    } finally {
    ...
    <div className="space-y-12 pb-20">
      ...
      <div className="space-y-6">
        ...
        <div className="p-6 rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900/50 flex flex-wrap gap-4 items-end">
          ...
          <button onClick={generateCode} disabled={isGenerating} className="px-6 py-2 text-sm font-medium text-white bg-zinc-900 dark:bg-zinc-100 dark:text-zinc-900 rounded-xl">生成兑换码</button>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          <AnimatePresence mode="popLayout">
            {codes.map((code) => <motion.div key={code.code} layout initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className={cn("p-4 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900/50 flex items-center justify-between group", code.isUsed && "opacity-50")}><div className="space-y-1"><div className="flex items-center gap-2"><span className="font-mono font-bold text-sm">{code.code}</span>{code.isUsed && <span className="px-2 py-0.5 text-[8px] font-bold uppercase tracking-wider rounded-full bg-zinc-100 text-zinc-500 dark:bg-zinc-800 dark:text-zinc-400">已使用</span>}</div><div className="text-[10px] text-zinc-500 uppercase tracking-wider font-medium">{code.type} • {code.value.toLocaleString()} Tokens</div><div className="text-[10px] text-zinc-400">{code.usedBy ? `使用者: ${code.usedBy}` : '未使用'}</div></div><div className="flex gap-1"><button onClick={() => copyCode(code.code)} className="p-1.5 hover:text-zinc-900 dark:hover:text-zinc-100 transition-colors">{copiedCode === code.code ? <Check className="w-3.5 h-3.5 text-emerald-500" /> : <Copy className="w-3.5 h-3.5" />}</button><button onClick={() => deleteCode(code.code)} className="p-1.5 text-zinc-400 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-all"><Trash2 className="w-3.5 h-3.5" /></button></div></motion.div>)}
          </AnimatePresence>
        </div>
      </div>
